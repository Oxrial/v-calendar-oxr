...
import { isUndefined, mapValues, toPairs, isFunction, has } from 'lodash-es'
...
Vue.mixin({
    computed: {
        $screens() {
            return (config, def) =>
                screensComp.matches.reduce((prev, curr) => (has(config, curr) ? config[curr] : prev), isUndefined(def) ? config.default : def)
        }
    },
    beforeCreate() {
        if (!isSettingUp) {
