...
export default class DateInfo {
    constructor(config, { order = 0, locale, isFullDay } = {}) {
        ...
        if (config.on) {
            const or = (isArrayLikeObject(config.on) ? config.on : [config.on])
                .map(o => {
    ...
    static get patterns() {
        return {
            ...
            weekdays: {
                validate: weekdays => (isArrayLikeObject(weekdays) ? weekdays : [parseInt(weekdays, 10)]),
            ...
            ordinalWeekdays: {
                validate: ordinalWeekdays =>
                    Object.keys(ordinalWeekdays).reduce((obj, ck) => {
                        ...
                        obj[ck] = isArrayLikeObject(weekdays) ? weekdays : [parseInt(weekdays, 10)]
                        return obj
                    }, {}),
